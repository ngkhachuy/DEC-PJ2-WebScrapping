...
if __name__ == '__main__':
    ...
    axbig1.set(ylabel=None, xlabel='Number of Product')

    # 02. Distribution of Price
    current_price = data[(data['CURRENT_PRICE'] > 0)]
    current_price_under4000 = current_price[(current_price['CURRENT_PRICE'] <= 2500)]
    current_price_over4000 = current_price[(current_price['CURRENT_PRICE'] > 2500)]
    print(current_price['CURRENT_PRICE'].describe())

    sns.histplot(current_price_under4000, x='CURRENT_PRICE', ax=ax[1, 0])
    ax[1, 0].set_title('Distribution of price (under $2500)')
    sns.histplot(current_price_over4000, x='CURRENT_PRICE', ax=ax[1, 1])
    ax[1, 1].set_title('Distribution of price (over $2500)')
    ...
    axbig2.set(ylabel=None, xlabel='Distribution of price')
